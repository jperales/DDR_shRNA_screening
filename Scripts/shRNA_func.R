### TITLE : R functions for the shRNA screen HT-seq analysis
### AUTHOR: Perales-Paton, Javier
### May2015 - TBU CNIO


# #---- Normalization
cpm.ByPool <- function(x, shRNA_lib) {
  x <- as.matrix(x)
  stopifnot(all(rownames(x)==unique(shRNA_lib$ID)))
  lib <- sapply(unique(shRNA_lib$batch),function(z) shRNA_lib[which(shRNA_lib$batch==z),"ID"])
  
  for(pool in names(lib)) {
    x.split <- x[lib[[pool]],]
    lib.size <- colSums(x.split)
    # lib.size <- 1e-06 * lib.size
    x[lib[[pool]],] <- t(t(x.split)/lib.size)
  }
  x <- t(t(x)/1e-6)
}
...
#---- Load data #######
readCounts <- function(listOfFiles,sampleNames,listOfTags,pathDIR=NULL) {
  if(!all(names(listOfFiles)%in%sampleNames)) {
    stop("ERROR: names of file list do _NOT_ match with sampleNames");
  }
  if(any(duplicated(listOfTags))){
    stop("ERROR: There is duplicated tags.");
  }
  
  count.df <- matrix(NA,nrow=length(listOfTags),ncol=length(sampleNames),
                      dimnames=list(listOfTags,sampleNames))
  count.df <- as.data.frame(count.df)
  for(s in sampleNames) {
    fl <- paste0(pathDIR,"/",listOfFiles[[s]])
    if(file.exists(fl)) {
      df.tmp <- read.table(fl,sep="\t",header=FALSE,
                           col.names = c("ID","count"),
                           colClasses = c("character","numeric"),
                           stringsAsFactors = FALSE)
      if(any(duplicated(df.tmp$ID))) {
        stop(paste0("ERROR: There is some duplicated tags."))
      } else {
        rownames(df.tmp) <- df.tmp$ID
      } 
      if(!all(rownames(count.df)%in%rownames(df.tmp))) {
        stop(paste0("ERROR: There is all tags represented."))
      } else { # Reorder the data.frame
        df.tmp <- df.tmp[rownames(count.df),]
      }
      # Fill the count.df
      count.df[,s] <- df.tmp[rownames(count.df),"count"] 
    } else {
      stop(paste0("ERROR: There is no file called ",fl))
    }
  }
  return(count.df)
}
...
#----- Plotting
barcodeplot2 <- function (statistics, index = NULL, index2 = NULL, gene.weights = NULL, 
                          weights.label = "Weight", labels = c("Up", "Down"), quantiles = c(-1, 
                                                                                            1), col.bars = NULL, worm = TRUE, span.worm = 0.45, ...) 
{
  ...
  if (!pos.dir) {
    ...
    if (nneg) 
      rect(n - nneg + 0.5, rect.yb, n + 0.5, rect.yt, col = "lightblue", 
           border = NA)
    segments(r, barlim[2]/2, r, barlim[2], lwd = lwd, col = "black")
    segments(r, barlim[2]/2 - shift, r, barlim[2]/2 * (1 + 
                                                         len.up) - shift, lwd = lwd, col = col.bars)
  }
  if (pos.dir) {
    rect.yb <- -0.5
    if (!TWO) 
    ...
    if (nneg) 
      rect(n - nneg + 0.5, rect.yb, n + 0.5, rect.yt, col = "lightblue", 
           border = NA)
    segments(r, barlim[1], r, barlim[2]/2, lwd = lwd, col = "black")
    segments(r, barlim[2]/2 + shift, r, barlim[2]/2 * (1 + 
                                                         len.up) + shift, lwd = lwd, col = col.bars)
  }
  if (TWO) {
  ...
  if (worm) {
    ...
    if (!TWO) {
      lines(x = 1:n, y = worm1.scale, col = "black", # Here is the color worm
            lwd = 2)
      ...
      axis(side = 2, 
           labels = "", # Deleted: labels = "enrichment"
           at = 1.6 + shift * pos.dir, padj = -0.6, tick = FALSE, cex.axis = 0.6)
    }
    if (TWO) {
      lines(x = 1:n, y = worm1.scale, col = col.bars, 
            lwd = 2)
      ...
      axis(side = 2, 
           labels = "", # Deleted: labels = "enrichment"
           at = 1.6 + shift, tick = FALSE, padj = -0.6, cex.axis = 0.5)
      axis(side = 2, 
           labels = "", # Deleted: labels = "enrichment"
           at = -1.6 - shift, tick = FALSE, padj = -0.6, cex.axis = 0.5)
    }
